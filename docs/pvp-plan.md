...
| P48 | Readying up | **You pick your formation and style, then press Ready**, and the lobby shows who has. The plan previously had no gate at all between choosing a shape and the host pressing Start, so a host could start on a player who had chosen nothing and the plan never said what that player got. **Ready is a signal, not a lock**: once the room is full the host may start whenever they like, and anyone not ready is given a default formation and style. Nobody can hold a room hostage by wandering off, and it needs no second clock. *Amended 2026-09-25: the SERVER rule is unchanged - the referee still takes a lineup from a ready player and the host may still start over one who never pressed it - but **the client settles the shape once you press Ready**, because a screen saying "I'm ready" above live formation chips is two answers to the same question. Not ready is the way back, and it is one tap* |
...
| P57 | When leaving a duel starts to cost | **The line is the DEAL, not the seat** (2026-09-02, reported from the game). P54 made walking out a forfeit from the moment somebody took the seat, and that is a whole phase too early: *"when a player enters a room via invitation and takes his seat, walking out of the room is directly counted as a loss. That's too early."* In the lobby nothing has been dealt and nothing bought, so neither player has yet seen anything there could be any advantage in rejecting - the person who opened it calls it off and the link dies with it, anybody else hands the seat back and the challenge goes on waiting, and neither costs a thing. From the instant the draft starts it is a forfeit at either end, exactly as P54 says. **It does not weaken P54 by a day**: the exploit P54 exists to shut off needs a squad ON THE SCREEN, and a squad arrives when the draft starts, so the free exit now ends precisely where the thing worth rejecting begins. The considered alternative was the reporter's own reading, free until the match is LIVE: rejected because a rolled duel deals its squads at the START of the draft, so that is the same free re-roll under another name, and because a budget duel having nothing random to re-roll does not buy enough to pay for a rule that differs by draft method. **The client half must never be deployed ahead of the referee**: a button saying "free" over a server that still charges costs somebody a game, where the reverse only over-warns |
...
| P22 | Display names | **Unique and changeable, no word filter, and the HOST is what a bad name is answered with.** *Amended 2026-09-02: reporting is GONE, and the whole feature with it - the flag on the seat row, `state/pvp/records.ts`'s insert and the `pvp_name_reports` table (dropped by migration 0026). It was decided before the host could do anything at all about another player, and it was the weaker half of the answer by every measure once 0025 shipped: a removal is immediate, it is the decision of the person who opened the room, it happens in the room where the problem is, and it needs nobody to read a queue by hand. A queue only the owner ever read did nothing about the lobby you were sitting in at the time, and the button said "Reported" while nothing happened. Two things go with it, both accepted: a name nobody hosts a room against is never seen by the owner at all, and a non-host has no lever but leaving.* *Amended 2026-08-31: a name can be CHANGED, and the rule that it is one per person is deliberately kept rather than moved onto the email address. The two questions are separate: the address is the account's identifier and now carries its own unique index (migration 0023), while the name is what a lobby shows, and a lobby with two Marios in it is the cheap grief this decision exists to prevent. Changing one needed no server work - `set_display_name` refuses a key held by somebody ELSE, so re-claiming your own row is the rename, and no room stores a name (every screen joins `profiles`), so a change reaches a room in flight.* *Amended 2026-08-26: uniqueness is on a **normalised key** (NFC, casefolded, whitespace collapsed, zero-width stripped, a defined codepoint set, 3 to 16 characters), not on raw text. Otherwise `Mario`, `mario`, `Mario ` and a Greek-omicron `Mariο` all coexist and are indistinguishable in a lobby, which is the cheap grief in a game whose only moderation is the owner renaming an account by hand. Records key on the account, so a rename is free* |
...
- **The records and the report do NOT go through the referee**, and that is the plan's own
  ...
  into. The referee is the only writer of ROOMS, and neither of these is a room. *(The
  report half went on 2026-09-02, table and grant included - see P22 - so the client now
  inserts into no table at all anywhere in the game, and only the record is left here.)*
