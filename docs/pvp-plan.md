...
- **"ACCEPTING STARTS THE DRAFT" WAS WRONG, AND THE DOCUMENT ARGUED FOR IT.** The reasoning
  read well: a duel has nobody to wait with, so a Ready button is one player pressing
  something and then leaving and a host's Start is a second visit for no decision, therefore
  `joinRoom` starts the room the instant the second seat is taken. It rested on one clause -
  "both shapes are already chosen by then, the challenger's when they sent it and the
  opponent's as they accept" - **and that clause was true of no code at all**. The formation
  control lives in `RoomLobby`, a duel replaces `RoomLobby` with its own panel, and the
  recipient was drafting before they ever saw a screen. So NEITHER side could pick a shape
  and every duel was played 4-3-3 balanced by both. Reported from the game on 2026-08-30.
  The fix keeps everything the reasoning got right and drops the false premise: there is
  still no host's Start, because the host may be asleep - the SERVER starts the room, in
  `tickDuel`, once every member is ready. The challenger is ready from creation so a
  challenge sent and abandoned cannot deadlock an accepted duel, and `ShapePicker` is shared
  with `RoomLobby` rather than copied.
  **The lesson generalises past duels, and it is the second time this file has taught it:**
  a design note that asserts a capability exists somewhere else is a note that has to be
  CHECKED against that somewhere else. This one named the exact screens it was wrong about.
